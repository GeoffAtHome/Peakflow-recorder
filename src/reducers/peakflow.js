/**
@license
Copyright (c) 2018 The Polymer Project Authors. All rights reserved.
This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
The complete set of contributors may be found at http://polymer.github.io/CONTRIBUTORS.txt
Code distributed by Google as part of the polymer project is also
subject to an additional IP rights grant found at http://polymer.github.io/PATENTS.txt
*/
import { PEAKFLOWRECORD_ID, PEAKFLOW_DATA_LOADED } from '../actions/peakflowrecords.js';
import { store } from '../store.js';
import { firebase } from '@firebase/app';
import '@firebase/database';
const INITIAL_STATE = {
    streetname: '',
    index: ''
};
const peakflowRecordMap = (state = INITIAL_STATE, action) => {
    switch (action.type) {
        case PEAKFLOWRECORD_ID:
            getPolygon(action._index);
            return Object.assign(Object.assign({}, state), { _index: action._index });
        case PEAKFLOW_DATA_LOADED:
            return Object.assign(Object.assign({}, state), { pos: action._data.pos, polygon: action._data.polygon });
        default:
            return state;
    }
};
async function getPolygon(key) {
    const data = await getPolygonFromFirebase(key);
    store.dispatch(publicStreetDataLoaded(data));
}
async function getPolygonFromFirebase(key) {
    const dataref = firebase.database().ref('/Polygons/' + key);
    const datasnapshot = await dataref.once('value');
    const polygon = datasnapshot.val();
    const data = {
        'pos': polygon.pos,
        'polygon': polygon.polygon,
        'index': key,
        'streetname': '',
        'groupid': ''
    };
    return data;
}
export default peakflowRecordMap;
// Per Redux best practices, the shop data in our store is structured
// for efficiency (small size and fast updates).
//
// The _selectors_ below transform store data into specific forms that
// are tailored for presentation. Putting this logic here keeps the
// layers of our app loosely coupled and easier to maintain, since
// views don't need to know about the store's internal data structures.
//
// We use a tiny library called `reselect` to create efficient
// selectors. More info: https://github.com/reduxjs/reselect.
export const peakflowRecordSelector = (state) => state.peakflowRecordMap;
